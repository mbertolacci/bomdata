...
test_that('load_rainfall, site_number variant', run_site_test(function(db_connection) {
    load_rainfall(db_connection, site_number = 3003)
}))

test_that('load_rainfall, site_data variant', run_site_test(function(db_connection) {
    load_rainfall(db_connection, site_data = get_site_raw(3003))
}))

test_that('load_rainfall, rainfall_data variant', run_site_test(function(db_connection) {
    rainfall_data <- get_rainfall_raw(get_site_raw(3003))
    load_rainfall(
        db_connection,
