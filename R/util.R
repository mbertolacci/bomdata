...
HIGH_QUALITY_RAINFALL_URL <- 'ftp://ftp.bom.gov.au/anon/home/ncc/www/change/HQdailyR/HQ_daily_prcp_txt.tar'
...
#' @export
load_high_quality_rainfall <- function(db_connection, tar_filename = NULL, url = HIGH_QUALITY_RAINFALL_URL) {
    if (is.null(tar_filename)) {
        futile.logger::flog.debug('Downloading high quality rainfall database from %s', url, name = 'bomdata.util')
    ...
    untar(tar_filename, exdir = output_directory)

    site_data <- read.fwf(file.path(output_directory, 'HQDR_stations.txt'), widths = c(7, 7, 7, 7, 200))
    site_data$p_c <- -1
    colnames(site_data) <- c('number', 'latitude', 'longitude', 'elevation', 'name', 'p_c')
    ...
    for (site_number in site_data$number) {
        futile.logger::flog.debug('Loading data for %d', site_number, name = 'bomdata.util')

        rainfall_z_filename <- file.path(output_directory, sprintf('prcphq.%06d.daily.txt.Z', site_number))
        system2('uncompress', rainfall_z_filename)

        rainfall_filename <- file.path(output_directory, sprintf('prcphq.%06d.daily.txt', site_number))

        raw_data <- read.fwf(rainfall_filename, skip = 1, widths = c(9, 9))
