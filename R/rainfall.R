...
download_rainfall <- function(site_data, url = RAINFALL_URL) {
  ...
  futile.logger::flog.debug(
    'Downloading site rainfall zip from %s',
    rainfall_url, name = 'bomdata.rainfall'
  )
  ...
  data <- data[, 3 : 8]
  colnames(data) <- c(
    'year', 'month', 'day', 'rainfall', 'days_measured', 'quality'
  )
  data$date <- sprintf(
    '%d-%02d-%02d',
    strtoi(data$year),
...
#' Load the rainfall data for a site into the database. The site must already
...
add_rainfall <- function(db_connection, site_number = NULL, site_data = NULL,
  ...
  return(DBI::dbWriteTable(
    db_connection, 'bom_rainfall', rainfall_data, append = TRUE
  ))
}
